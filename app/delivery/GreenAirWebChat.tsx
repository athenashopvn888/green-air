...
export default function GreenAirWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Green Air Cannabis chat could not be restored."));
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storeId: "GAC01", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, workflowVersion: "READY_V1", smsConsent }) }));
  ...
  return <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Green Air Cannabis Web Chat">
    ...
    {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Green Air Cannabis Web Chat">
      <header><div><strong>Green Air Cannabis Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">Ã—</button></header>
      ...
      {!token ? (!availability || availability.state !== "AVAILABLE" ? <div className="sod-chat-start sod-chat-paused"><p>{statusMessage || "Checking delivery availability..."}</p><button type="button" onClick={() => void refreshAvailability()}>Check again</button></div> : <form className="sod-chat-start" onSubmit={start}>
        ...
        {intent && <><label>Canadian mobile number<input required inputMode="tel" value={phone} onChange={(event) => setPhone(event.target.value)} autoComplete="tel" placeholder="647 555 0123" aria-describedby={intent === "NEW_CUSTOMER" ? "sod-phone-help" : undefined} />{intent === "NEW_CUSTOMER" && <small id="sod-phone-help">Must be able to receive verification texts. This becomes your account number.</small>}</label>
          ...
          <label>Order details (optional)<textarea maxLength={1000} value={firstMessage} onChange={(event) => setFirstMessage(event.target.value)} placeholder="What would you like to order today?" /></label><button type="submit" disabled={busy || !smsConsent}>{busy ? "Startingâ€¦" : "Start order chat"}</button></>}
