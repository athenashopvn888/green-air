...
import GreenAirWebChat from "./GreenAirWebChat";
...
export default function DeliveryContent() {
  ...
  async function handleEmailSubmit(event: React.FormEvent) {
    ...
    try {
      await fetch(`https://script.google.com/macros/s/AKfycbySrZYxI-NNnXfxY1jXOqHgT2HQi4zst2Fgte6FXTeymat_W_r0o1E3P83EfnVCjEk0/exec?action=delivery_email&email=${encodeURIComponent(email)}&store=GAC01`, { method: "GET", mode: "no-cors" });
    } finally {
  ...
  useEffect(() => {
    ...
    fetch("https://milestone-1-demo.vercel.app/api/catalog?store=GAC01", { signal: controller.signal })
  ...
  return <main className={styles.main}>
    ...
    <section className={`${styles.hero} ${styles.heroPlain}`}>
      <div><p>Green Air Cannabis · Malton / Airport Rd</p><h1>Delivery Menu</h1><span>Dispatcher-confirmed delivery for Malton, Airport Road, and nearby Mississauga addresses. Neighbourhood delivery notes live on <a href="/cannabis-delivery-malton" style={{color:"inherit",fontWeight:800}}>cannabis delivery in Malton</a>. Walk-in NAP stays on the homepage at 7060 Airport Rd.</span></div>
    </section>
    ...
    <section className={styles.howToOrder} aria-labelledby="how-to-order-title"><div><p>HOW TO ORDER</p><h2 id="how-to-order-title">Order with the Green Air Cannabis dispatcher</h2><span>LIVE ORDER connects you with the Green Air Cannabis dispatcher.</span></div><ol><li><strong>Browse the delivery menu</strong><span>Note the product names and weights you want.</span></li><li><strong>Select LIVE ORDER</strong><span>Open Web Chat at the bottom-right and send your choices.</span></li><li><strong>Verify privately if you are new</strong><span>New customers complete the private selfie-with-ID step in Web Chat.</span></li><li><strong>Confirm with the dispatcher</strong><span>The dispatcher confirms availability, delivery details, and next steps.</span></li></ol></section>
      ...
      <div className={styles.catalog}>
        ...
        <div className={styles.grid}>{visible.map((product) => <article className={styles.card} key={product.publicProductId}>
          <button type="button" className={styles.imageButton} onClick={() => setSelected(product)} aria-label={`View details for ${product.name}`}>
            {product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on the Green Air Cannabis delivery menu`} fill sizes="(max-width: 640px) 50vw, 280px" unoptimized /> : <span>Green Air Cannabis</span>}
          </button>
    ...
    <section className={styles.updates} aria-labelledby="delivery-updates-title">
      <div><p>DELIVERY UPDATES</p><h2 id="delivery-updates-title">Get store delivery updates</h2><span>Enter your email to keep the existing Green Air Cannabis delivery notification active.</span></div>
    ...
    <div className={styles.ctaSection}><p>Visit us in-store at <strong>7060 Airport Rd, Mississauga</strong>. We are <strong>open 24 hours daily</strong>.</p></div>
